...
class HX711:
    ...
    def getMedian(self, times=3):
       if times <= 0:
          raise ValueError("HX711::getMedian(): times must be greater than zero!")
    ...
    def getAverage(self, times=3):
        # Make sure we've been asked to take a rational amount of samples.
        if times <= 0:
            raise ValueError("HX711()::getAverage(): times must >= 1!!")
    ...
    def tare(self, times=15, channel='A'):
        if channel == 'A':
            backupReferenceUnit = self.REFERENCE_UNIT_A
        elif channel == 'B':
            backupReferenceUnit = self.REFERENCE_UNIT_B
        else:
            raise ValueError("HX711::tare() invalid channel: \"%s\"" % channel)
        
        self.setReferenceUnit(1, channel)
            
        value = self.getAverage(times)

        self.setOffset(value, channel)

        self.setReferenceUnit(backupReferenceUnit, channel)
